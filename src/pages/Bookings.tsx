    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500">{filtered.length} bookings</p>
        </div>
          ...
          <>
            <Table>
              ...
              <TBody>
                {paginated.map((b) => (
                  <TR key={b.id}>
                    <TD className="font-medium text-slate-900">{b.booking_number}</TD>
...
function SortableTH({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <TH>
      <button onClick={onClick} className="flex items-center gap-1 hover:text-slate-700">
        {label}
